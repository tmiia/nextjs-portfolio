...
void main() {
  ...
  float offset = mix(-0.25, 0.25, uv.y);
  ...
  for(int i = 0; i < 4; i++) {
    ...
    float noiseVal = smoothstep(noiseFloor, noiseCeil, snoise(
      vec3(
        noiseCoord.x * noiseFreq.x + (uTime * 0.003) * noiseFlow,
        noiseCoord.y * noiseFreq.y,
        (uTime * 0.003) * noiseSpeed + noiseSeed
      )
    ));

    vColor = mix(vColor, uColor[i], noiseVal);
  }
